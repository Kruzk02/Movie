...
public class UserServiceImpl implements UserService {
    ...
    public Mono<String> login(UserDTO userDTO) {
        var authRequest = new UsernamePasswordAuthenticationToken(userDTO.getUsername(), userDTO.getPassword());

        return reactiveAuthenticationManager.authenticate(authRequest)
                .flatMap(authentication -> {
                    String authenticatedUsername = authentication.getName();
                    return Mono.just(jwtUtil.generateToken(authenticatedUsername));
                })
                .onErrorResume(ex -> Mono.error(new AuthenticationFailedException("Login failed: " + ex.getMessage())));
    }
